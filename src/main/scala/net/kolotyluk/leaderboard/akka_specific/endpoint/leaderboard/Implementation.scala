...
import net.kolotyluk.leaderboard.akka_specific.endpoint.leaderboard.failure.DupicateLeaderboardIdentifierError
...
object Implementation extends Enumeration with Logging {
  ...
  val LeaderboardActor = Val(
    leaderboardIdentifier => {
      ...
      val leaderboardFuture = net.kolotyluk.leaderboard.akka_specific.leaderboardManagerActor.create(leaderboardIdentifier)
